...
import { content, isWebUrl, isConfigured, releaseIssues } from './src/content.ts';
const escapeHtml = (s: string) => s.replace(/[&<>"']/g, c => ({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;'}[c]!));
export default defineConfig({
  plugins: [react(), {
    name: 'content-seo',
    transformIndexHtml(html) {
      const domain = isWebUrl(content.links.domain) ? new URL(content.links.domain).origin : '';
      const title = isConfigured(content.owner.name) ? `${content.owner.name} · ${content.seo.title}` : content.seo.title;
      const schema = { '@context': 'https://schema.org', '@type': 'WebSite', name: title, description: content.seo.description, inLanguage: 'es-MX', ...(domain ? {url: domain} : {}) };
    ...
    generateBundle() {
      ...
      if (ready && domain) this.emitFile({type:'asset',fileName:'sitemap.xml',source:`<?xml version="1.0" encoding="UTF-8"?><urlset xmlns="http://www.sitemaps.org/schemas/sitemap/0.9"><url><loc>${escapeHtml(domain)}/</loc></url></urlset>`});
    },
  }],
